...
import './SliderProductosDestacadosStyle.css';
...
const SliderProductosDestacados = () => {
  ...
  return (
    <div className="container mt-2 margin-bottom">

    <div className="titulo-slider" id="productos">
        <h1 className="py-3 my-4 titulo-principal1">¡Productos interesantes para ofrecer!</h1>
        <p className="enlace-ver-todos"></p>
    </div>
        
        <Slider {...sliderSettings} ref={customeSlider} className="slider-contenedor">
        {/* <div onClick={()=>gotoPrev()}><GrPrevious className='boton-slider-prev'/></div>
        <div onClick={()=>gotoNext()}><GrNext className='boton-slider-next'/></div> */}
            {proddestacado.map((product, index)=>(
            <Card key={index} className="card-all-slider">
          ...
          {userInfo && (
            ...
            {product.destacado && (
            <>
              <p className="estrella-destacado-slider"></p>
              <p className="texto-destacado-slider">PRODUCTO DESTACADO <AiFillStar className="estrella"/></p>
            </>
            )}
...
export default SliderProductosDestacados