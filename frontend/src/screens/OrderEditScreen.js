...
export default function OrderEditScreen () {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        // setModifiedCount(data.status.history.modifiedCount + '1');

        // if((data.status.history.modifiedTicket.countTicket + 1)) {
            // setcountTicket(data.status.history.modifiedTicket.countTicket + '1');
        // }
  ...
  }, [order, userInfo, orderId, navigate, successPay, successDeliver, cuposolicitado, idproducto]);
  ...
  useEffect(() => {
    ...
    const fetchStock = async () => {
      ...
      try {
        ...
        // setModifiedstockdays(data[0].pasajerosmodified);
        // setPasajeros(data[0].pasajeros)
  ...
  }, [idproducto, userInfo.token, fecha, refetchStock ]);
    ...
    if(window.confirm(`¿Estás seguro que deseas modificar la reserva? ${order.shippingAddress.servicio}`)) {
      ...
      try {
        ...
        const { data } = await axios.put(
          `/api/orders/${order._id}/modificacion`,
            {
            fechaModificacion, 
            servicio,
            fecha,
  ...
  // console.log(fechaServicio1);

  // const footer = fecha && 
  //   <div className="mt-3">
  //     <p className="fecha-cupo-pie-de-calendario fs-2"><strong>{fecha.toLocaleString('es-ES', {dateStyle: 'full'})}</strong></p>
  //     {/* <button disabled={mostrar} id="boton-cupo" className="boton-verificar">Verificar</button> */}
  //   </div>
    ...
    <div className="container extra-container">
      ...
      <div className="contenedor-titulo-editar-reserva">
        ...
        <div className="d-flex align-items-center">
          ...
          <div className="contenedor-enlace-refresh-orden" onClick={(e)=> refetchOrder(e)}>
      ...
      <div id="up"  className="grid-formulario">
        ...
        <div className="container bloque-form1">
              {/* <Form.Group className="contenedor-calendario" controlId="fecha">
                <Form.Label className="mb-1 fs-5 label label-calendario">
                    *Si modificás la fecha, verificá bien el cupo disponible.
                </Form.Label>
                      <DayPicker 
                          // type="date"
                          name="fecha"
                          ...
                          // modifiersStyles={fecha ? ({ booked: bookedStyle }) : ({ booked: conStock})}
                          modifiersStyles={stockinicial - cupofinal === 18 ? ({ booked: bookedStyle }) : ({ booked: conStock})}
